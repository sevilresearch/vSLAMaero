...
'''
According to Luke,
    u, u2, u3, are the UAV's positions: x = [0], y = [1], theta??? = [2]
    a, b are the desired waypoints for each drone
'''
def calEntropy (u,u2,u3,a,b,Vmax,flag):
    x = np.zeros(3,dtype= float)    # array contains the x positions of the drones
    y = np.zeros(3,dtype = float)   # array contains the y positions of the drones
    t = np.zeros(3,dtype = float)   # array contains the angle (relative to what???) of the drones
    d = np.zeros(3,dtype = float)   # array contains the distance each drone is from the first
    Th = np.zeros(3,dtype = float)  # array contains the angle (relative to the first drone???) of the drones
    V_com = 0.0     # desired velocity
    theta_com = 0.0     # desired angle
    e = np.zeros(3,dtype = float)   # Entropy???
    

    '''
    Should be able to be more easily specified
    '''
    ...
    '''
    Contains the first drone's data
    theta relative to what?
    '''
    x_cur = u[0]
    y_cur = u[1]
    theta_cur = u[2]
    ...
    '''
    calculating each drone's distance and angle from the first drone
    '''
    ...
    '''
    Finding the closer of the two drones
    '''
    if d[1] < d[2]:
        minimum=d[1]
        maximum=d[2]
    ...
    '''
    Range 3 for the num of drones here
    We want entropy to be as low as possible so if the drone is too far, set entropy of the ith drone to max
    Settings the entropy to be the distance the drone is
    '''
    for i in range(3):
    ...
    #print("D's: ", d)
    #print("E's: ", e)
    #print("Summation = ", summation, " { ", (e[0]/maxDistance)**q, " + ", (e[1]/maxDistance)**q , " + ", (e[2]/maxDistance)**q , "}")

    # St = ((1-summation)/(1-q))
    '''
    We now have the entropy. should stop here
    '''
    St = ((1-summation)/(q-1))#7/1/20
    St_temp = St

    '''
    The rest of this is determining the next move which should be handled else where
    '''
